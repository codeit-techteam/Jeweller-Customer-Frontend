...
const GOLD_PLAN_ITEMS: {
  label: string;
  sub: string;
  icon: IconName;
  planId: ComingSoonPlanId;
  variant: 'mine' | 'reserve';
}[] = [
  { label: 'Gold Mine', sub: '10+1 Monthly Plan', icon: 'diamond', planId: 'gold_mine', variant: 'mine' },
                ...
                >
                  ...
                  <View style={styles.menuList}>
                    {GOLD_PLAN_ITEMS.map((item) => (
                      <Pressable
                        key={item.label}
                        onPress={() => openComingSoon(item.planId)}
                        style={({ pressed }) => [
                          styles.menuItemOuter,
